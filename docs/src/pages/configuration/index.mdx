...
- Click on your model, then on the next page, click the `Create deployment` button. Select `Online` for the deployment type, provide a name for the deployment and click `Create`. After a while, your scoring service is now deployed as a web service. Click on the service to get the endpoint to execute the scoring. You can also perform some quick tests from this page.
![](../images/deployed-service-details.png)

**Important**: Take note of the endpoint provided in the above page, as it is the one that you will provide to the ODM decision service so it can invoke the scoring service.
